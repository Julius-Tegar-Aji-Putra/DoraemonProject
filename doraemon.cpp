...
void displayControlInfo() {
    ...
    if (cameraMode == 0) {
        panduanStr += " || W/S: Maju/Mundur || A/D: Rotasi Kiri/Kanan || Q/E: Naik/Turun";
    } else {
...
// Fungsi untuk orientasi kamera (menggunakan camAngle dalam degree)
void orientCamera() { 
    float ang_rad = camAngle * M_PI / 180.0f;
    camLookX = sin(ang_rad);
    camLookZ = -cos(ang_rad); 
}
...
static void processKeys() {
    // Speeds for Doraemon (mode 1)
    float doraemonSpeed = 0.04f;
    float doraemonRotationSpeed = 0.6f; 
    float maxTilt = 15.0f;
    ...
    if (cameraMode == 0) {
        float camMoveSpeed = 0.04f;
        float camRotationSpeed = 0.6f; // degrees
        float camVerticalSpeed = 0.04f;

        // Handle Rotation first (A/D) as it affects movement direction
        if (keyStates['a'] || keyStates['A']) {
            camAngle -= camRotationSpeed;
            orientCamera();
        }
        ...
        // Calculate movement delta based on current orientation for W/S
        float moveX = 0.0f;
        float moveZ = 0.0f;

        if (keyStates['w'] || keyStates['W']) {
            moveX += camMoveSpeed * camLookX;
            moveZ += camMoveSpeed * camLookZ;
        }
        ...
        // Potential new horizontal position
        float nextFreeCamX = freeCamX + moveX;
        float nextFreeCamZ = freeCamZ + moveZ;

        // Horizontal collision check (arena walls)
        float responseX = 0.0f;
        float responseY = 0.0f; // Not used by current checkCameraCollision for XZ walls
        float responseZ = 0.0f;

        // Gunakan freeCamY saat ini untuk pengecekan tabrakan XZ
        if (checkCameraCollision(nextFreeCamX, freeCamY, nextFreeCamZ, &responseX, &responseY, &responseZ)) { //
            freeCamX = nextFreeCamX + responseX; // Terapkan koreksi X
            freeCamZ = nextFreeCamZ + responseZ; // Terapkan koreksi Z
        } else {
            freeCamX = nextFreeCamX; // Tidak ada tabrakan, perbarui posisi X
            freeCamZ = nextFreeCamZ; // Tidak ada tabrakan, perbarui posisi Z
        }

        // Vertical Movement (Q/E) with explicit Y-axis boundary checks
        float newFreeCamY = freeCamY;
        if (keyStates['q'] || keyStates['Q']) {
            newFreeCamY += camVerticalSpeed;
        }
        if (keyStates['e'] || keyStates['E']) {
            newFreeCamY -= camVerticalSpeed;
        }

        // Boundary checks for Y (floor and an assumed ceiling based on wallHeight)
        float cameraSphereRadius = 1.0f; // Radius kamera dari checkCameraCollision
        float arenaFloorY = 0.0f;
        // Ambil wallHeight dari arena.cpp (misalnya 100.0f, atau buat getter jika perlu)
        // Untuk sekarang, kita asumsikan wallHeight adalah 100.0f seperti di arena.cpp
        // Sebaiknya ini didapat dari fungsi getArenaWallHeight() atau konstanta global jika ada
        float arenaCeilingY = 100.0f; // Asumsi tinggi dinding arena dari arena.cpp

        if (newFreeCamY < arenaFloorY + cameraSphereRadius) {
            newFreeCamY = arenaFloorY + cameraSphereRadius;
        }
        if (newFreeCamY > arenaCeilingY - cameraSphereRadius) {
            newFreeCamY = arenaCeilingY - cameraSphereRadius;
        }
        freeCamY = newFreeCamY;
    } else { // cameraMode == 1 (Third Person)
        float newX = posX;
        float newY = posY;
        float newZ = posZ;
        bool moved = false;
        ...
        if (keyStates['w'] || keyStates['W']) {
            newX = posX + doraemonSpeed * sin(angleY * M_PI / 180.0f);
            newZ = posZ + doraemonSpeed * cos(angleY * M_PI / 180.0f);
            moved = true;
            targetTiltZ = maxTilt;
        }
...
void setCameraMode(int mode) {
    cameraMode = mode;
    if (cameraMode == 0) { // When switching to free cam, ensure orientation is up to date //
        orientCamera();
    }
}
...
bool checkCameraCollision(float cameraX, float cameraY, float cameraZ, float* responseX, float* responseY, float* responseZ) {
    ...
    *responseZ = 0.0f;
    ...
    float currentArenaSize = getArenaSize(); // Dapatkan ukuran arena saat ini
    float boundarySize = currentArenaSize / 2.0f;
...
void initDoraemon() {
    ...
    // Initialize free camera position and orientation
    freeCamX = 10.0;
    freeCamY = 8.0;
    freeCamZ = 10.0;
    camAngle = 0.0f; 
